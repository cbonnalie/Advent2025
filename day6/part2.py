...
def orient_equations(input_text, num_lengths):
    ...
    for equation in equations:
        operands.append(equation.pop()[0])

    transposed_equations = transpose(equations, num_lengths)

    return transposed_equations, operands


def transpose(equations, num_lengths):
    transposed = []
    for eq_index, equation in enumerate(equations):
        chunk = []
        length = num_lengths[eq_index]
        for digit_index in range(length):
            new_num = ''
            for num in equation:
                if num[digit_index] != ' ':
                    new_num += num[digit_index]
            chunk.append(int(new_num))
        transposed.append(chunk)
    return transposed


def solve_equations(equations, operands):
    sum = 0
...
def main():
    ...
    equations, operands = orient_equations(input_text, num_lengths)
    sum = solve_equations(equations, operands)
    print(sum)
